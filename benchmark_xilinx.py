...
#   -v VIDEO_QUALITY, --videoquality=VIDEO_QUALITY
#                     codec setings for Xilinx U30 visual quality optimizations <default, objective, subjective>
#                     default: default
...
def main():
    (filename, ofilename, split_count, input_encoder, output_encoder, bitrate, video_quality) = parse_options()
    if split_count < 1:
        print ("number of segments should be at least 1")
        raise SystemExit

    codec_settings = "-f mp4"

    if video_quality == "objective":
        codec_settings = "-f mp4 -bf 2 -g 120 -periodicity-idr 120 -qp-mode uniform -temporal-aq 1 -spatial-aq 0 -scaling-list 0"
    elif video_quality == "subjective":
        codec_settings = "-f mp4 -bf 1 -g 120 -periodicity-idr 120 -qp-mode relative-load -temporal-aq 1 -spatial-aq 1 -scaling-list 1"
    elif video_quality != "default":
        print ("video quality option needs to be objective, subjective or default")
        raise SystemExit
    ...
    #max supported resolution of lookahead_depth is 1920x1080 (or 1080x1920 portrait mode)
    if (xres <= 1920 and yres <= 1080 and video_quality != "default") or (xres <= 1080 and yres <= 1920 and video_quality != "default"):
        codec_settings += " -lookahead_depth 20"
    ...
    total_frame = video_length * framerate
    print("      resolution: "+ str(xres)+"x"+str(yres))
    print("      framerate: "+ str(framerate))
    print("      frame: "+ str(total_frame))
    ...
    for n in range(0, split_count):        
        transcode_cmd = "ffmpeg -loglevel info -xlnx_hwdev "+ str(n % 2)+" -vsync 0 -c:v mpsoc_vcu_" + input_encoder + " -i tmpfile" + \
                        format(n, '02d') + filename[-4:] + \
                        " " + codec_settings + \
                        " -b:v " + bitrate + " -max-bitrate " + \
                        bitrate + " -c:v mpsoc_vcu_" \
                        + output_encoder + " -y tmpfileout" + \
                        format(n, '02d') + ofilename[-4:] + " > stdout" +str(n)+".log 2>&1 & \n"

        print ("Start transcoding segment: " + str(n) + "with cmd: " + transcode_cmd)
    ...
    print(" ")
    print("fps: " + str(round(1.0*total_frame/totSec,1)) + " frames per second")
...
def parse_options():
    ...
    parser.add_option("-b", "--bitrate",
                      ...
                      help = "output bitrate in Mbit/s. Must be a float or integer value between 1.0 and 25.0 (example: use -b 3 to specify an output bitrate of 3Mbits/sec) \
                              default 5.0",
                      type = "string",
                      action = "store", default = "5M"
    )
    parser.add_option("-v", "--videoquality",
                      dest = "video_quality",
                      help = "codec setings for Xilinx U30 visual quality optimizations <default, objective, subjective> \
                              default default",
                      type = "string",
    ...
    if options.ifilename and options.ofilename and options.split_count:
        return (options.ifilename, options.ofilename, options.split_count, \
                options.input_encoder, options.output_encoder,options.bitrate, options.video_quality)
    else:
        parser.print_help()
        raise SystemExit
